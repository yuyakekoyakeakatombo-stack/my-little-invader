...
;globalThis.__api = {
  ...
  get playSel(){ return playSel; }, set playSel(v){ playSel = v; },
  FEED_ITEMS,
 ...
 S4_SPR, S4_SLEEP, S4_SHUT, S3_SHUT, G_LIE, CLAMP_SLIDE, eyeRows, sickSprite, charSprites, S3_SPR, S3_SLEEP, lie, withRows, BIRD_UP, BIRD_DOWN, BIRD_FAR_UP, BIRD_FAR_DOWN, BIRD_FAR_X, birdSprite, BIRD_FLAP, BIRD_SPEED, BIRD_GAP, SHOOT_CHANCE, SHOOT_LEN, BIRD_BOB,
  ...
  get shootStar(){ return shootStar; }, set shootStar(v){ shootStar = v; },
  get birds(){ return birds; }, set birds(v){ birds = v; },
  get nightUfo(){ return nightUfo; }, set nightUfo(v){ nightUfo = v; },
  get cloudX(){ return cloudX; }, set cloudX(v){ cloudX = v; },
  ...
  get walkDir(){ return walkDir; }, set walkDir(v){ walkDir = v; },
  get walkTimer(){ return walkTimer; }, set walkTimer(v){ walkTimer = v; },
  get jumpT(){ return jumpT; }, set jumpT(v){ jumpT = v; },
  get jumpDur(){ return jumpDur; }, get jumpHi(){ return jumpHi; },
  get floatT(){ return floatT; }, set floatT(v){ floatT = v; },
  get hopPause(){ return hopPause; }, set hopPause(v){ hopPause = v; },
  get preJumpT(){ return preJumpT; }, set preJumpT(v){ preJumpT = v; },
  get wiggleT(){ return wiggleT; }, set wiggleT(v){ wiggleT = v; },
  get squatT(){ return squatT; }, set squatT(v){ squatT = v; },
  get squatWalkT(){ return squatWalkT; }, set squatWalkT(v){ squatWalkT = v; },
  get lookT(){ return lookT; }, set lookT(v){ lookT = v; },
  get dashT(){ return dashT; }, set dashT(v){ dashT = v; },
  setReaction,
  get reactT(){ return reactT; }, set reactT(v){ reactT = v; },
  get reactType(){ return reactType; }, set reactType(v){ reactType = v; },
  get reactQuiet(){ return reactQuiet; }, set reactQuiet(v){ reactQuiet = v; },
  get reactFood(){ return reactFood; }, set reactFood(v){ reactFood = v; },
  get cleanSnapW(){ return cleanSnapW; }, set cleanSnapW(v){ cleanSnapW = v; },
  get cleanSnapPlate(){ return cleanSnapPlate; }, set cleanSnapPlate(v){ cleanSnapPlate = v; },
  get cleanSnapSpoiled(){ return cleanSnapSpoiled; }, set cleanSnapSpoiled(v){ cleanSnapSpoiled = v; },
  get cleanSnapPoopFly(){ return cleanSnapPoopFly; }, set cleanSnapPoopFly(v){ cleanSnapPoopFly = v; },
  get cleanSnapKind(){ return cleanSnapKind; }, set cleanSnapKind(v){ cleanSnapKind = v; },
  EAT_T, EAT_STEP, CLEAN_T, MED_FLY, MED_BLINK, MED_T, MED_FROM, MED_ARC, medPos, medTarget, MED_PILL,
  SKY_TOP, inSky, rainDrops, snowFlakes, updateRain, updateSnow, RAIN_STYLE,
  EMO_GAP, EMO_UP, POOP_X, PLATE_X, POOP_W, foodW, foodSet,
  ...
  healthState, statusAlert, isWeakStarve, isWeakSick, poopHasFlies, poopFlyAt, fitPoopBorn, needsMed,
  ...
  get titleWait(){ return titleWait; }, set titleWait(v){ titleWait = v; },
  get openingDone(){ return openingDone; }, set openingDone(v){ openingDone = v; },
  get openingReady(){ return openingReady; }, set openingReady(v){ openingReady = v; },
  ...
  KINSOKU, wrapStory, STORY_ARW_W, STORY_ARW_H, STORY_ARW_Y, storyPageCount,
  get storyPage(){ return storyPage; }, set storyPage(v){ storyPage = v; },
